...
if [ $# -eq 0 ]
	then
		...
		openssl s_client -showcerts -servername $1 -connect $1:443 2>/dev/null </dev/null > $c 
		# Use OpenSSL to connect to and reteive server certificates and output to temp file
		...
		sed -n '/-----BEGIN CERTIFICATE-----/,/-----END CERTIFICATE-----/p; /-----END CERTIFICATE-----/q' $c > $n 
		# Pull only PEM certificate and discard all other data, write to final output file
